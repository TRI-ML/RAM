...
class Detector(object):
  ...
  def run(self, image_or_path_or_tensor, meta={}):
    ...
    for scale in self.opt.test_scales:
      ...
      result, trans = self.post_process(dets, meta, scale)
    ...
    if self.opt.tracking:
      ...
      preproc_image_shape = images.shape[2:]
      
      results, heatmap = self.tracker.step(results, current_result['A'], self.debugger, [output['hm'].shape[2], output['hm'].shape[3]], raw_image_shape, preproc_image_shape, trans)
      for res in results:
        if 'walk' in res:
          res['walk'] = None
    ...
    if self.opt.debug >= 1:
      ...
      self.show_results(self.debugger, image, results, pred_hm, pre_hm, heatmap)
    self.cnt += 1
  ...
  def pre_process(self, image, scale, input_meta={}, out_path=None):
    '''
    ...
    with torch.no_grad():
      ...
      if self.stream_test:
        if self.opt.test_with_loss:
          ...
          output, self.h, A, self.prev_feat = self.model.step(batch_list, self.h, self.prev_feat)
      else:
        if self.opt.test_with_loss:
          output, loss, loss_stats, pre_hms, _, _, _ = self.model_with_loss(batch_list, batch_size=1, eval_mode=True)
        else:
          self.h = None
          self.prev_feat = None
          for i in range(len(batch_list)):
            output, self.h, A, self.prev_feat = self.model.step([batch_list[i]], self.h, self.prev_feat)
  ...
  def post_process(self, dets, meta, scale=1):
    dets, trans = generic_post_process(
  ...
  def show_results(self, debugger, image, results, hm, pre_hm=None, crw_heatmap=None):
    ...
    if crw_heatmap is not None:
      if crw_heatmap.max() > 0:
        crw_heatmap = (crw_heatmap / crw_heatmap.max())
      crw_heatmap = cv2.applyColorMap((crw_heatmap * 255).astype(np.uint8), cv2.COLORMAP_JET)
      debugger.add_blend_img(image, crw_heatmap, 'generic', trans=0.4)
