...
class GenericDataset(data.Dataset):
  ...
  def __init__(self, opt=None, split=None, ann_path=None, img_dir=None):
    ...
    if ann_path is not None and img_dir is not None:
      ...
      elif 'occl_episode' in image_info and split != 'val' and split != 'test':
        filtered = []
        for image_id in self.images:
          image_info = self.coco.loadImgs(ids=[image_id])[0]
          if image_info['occl_episode']:
            filtered.append(image_id)
          if 'is_carry' in image_info and image_info['is_carry']:
            for i in range(5):
              filtered.append(image_id)
  ...
  def _load_image_anns(self, img_id, coco, img_dir, random_flag):
  ...
  def _get_pre_dets(self, anns, trans_input, trans_output, apply_noise=True):
    ...
    pre_cts, pre_vs, track_ids, occl_lengths, pre_ids = [], [], [], [], []
    for ann in anns:
      box_size = ann['bbox'][2] * ann['bbox'][3]
      cls_id = int(self.cat_ids[ann['category_id']])
      if cls_id > self.opt.num_classes or cls_id <= -99 or cls_id == 0 or \
         ('iscrowd' in ann and ann['iscrowd'] > 0):# or (box_size < self.box_size_thresh):
        continue
      ...
      if (h > 0 and w > 0):
        ...
        #apply noise is used as a proxy for skipping the first frame
        if apply_noise and 'v' in ann:
          pre_vs.append(ann['v'])
        else:
          pre_vs.append([0, 0])
        ...
        if 'radius_mutiplier' in ann:
          occl_length = ann['radius_mutiplier']
    ...
    return pre_hm, pre_cts, track_ids, occl_lengths, pre_vs, pre_ids
  ...
  def _init_ret(self, ret, gt_det):
    ...
    ret['v'] = np.zeros(
          (max_objs, regression_head_dims['tracking']), dtype=np.float32)
  ...
  def _mask_ignore_or_crowd(self, ret, cls_id, bbox, track_id, pre_cts, pre_track_ids, gt_det):
    gt_det['ignore_boxes'].append(bbox)
    gt_det['ignore_classes'].append(cls_id)
  ...
  def _add_instance(
    ...
    aug_s, calib, pre_cts=None, track_ids=None, pre_vs=None, pre_ids=None, occlusion=None, radius_mutiplier=None, occl_lengths=None):
    h, w = bbox[3] - bbox[1], bbox[2] - bbox[0]
    if h <= 0 or w <= 0:
      return False
    radius = gaussian_radius((math.ceil(h), math.ceil(w)))
    ...
    if cls_id == 1:
      ret['mask'][k] = self.opt.person_weight
    ...
    if radius_mutiplier is not None:
      upper_bound = 1 + self.opt.radius_growth_step * 10
      radius_mutiplier = 1 + self.opt.radius_growth_step * radius_mutiplier
      radius *= min(radius_mutiplier, upper_bound)
  ...
  def _format_gt_det(self, gt_det, max_gts=1000):
    padded_gt_det = {'bboxes': np.array([[0,0,1,1]] * max_gts, dtype=np.float32), 
              ...
              'occlusion': np.array([0] * max_gts, dtype=np.float32),
              ...
              'ignore_boxes': np.array([[0,0,1,1]] * max_gts, dtype=np.float32),
              'ignore_classes': np.array([-1] * max_gts, dtype=np.int32)}
    
    for k in gt_det:
      if k not in padded_gt_det:
        continue
