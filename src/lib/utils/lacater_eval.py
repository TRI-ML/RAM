...
class ResultsAnalyzer(object):
    ...
    @classmethod
    def init_from_files(cls, bb_prediction_dir: str, bb_gt_dir: str, iou_thresh: float = None, max_vids: int = None):
        ...
        if max_vids is not None:
            bb_predictions = bb_predictions[:max_vids]
            bb_gt = bb_gt[:max_vids]
            video_files = video_files[:max_vids]
    ...
    def _compute_iou_results(self) -> None:
        ...
        for video_name in self.videos_names:
            ...
            if None in video_predictions:
                self.iou_results[video_name] = np.array([0.0] * 300)
                continue
