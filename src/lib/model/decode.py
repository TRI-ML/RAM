...
def get_scores(heat, do_sigmoid, K):
  if do_sigmoid:
    heat = _sigmoid(heat.clone().detach())
  ...
  return scores, clses, xs0, ys0, cts, inds

def get_wh(wh, cat, batch, inds, K, clses):
...
def generic_decode(output, K=100, opt=None, do_sigmoid=False):
  if not ('hm' in output):
    return {}
  ...
  bboxes = None
  scores, clses, xs0, ys0, cts, inds = get_scores(output['hm'], do_sigmoid, K)

  visibility = torch.ones(batch, K, 1).cuda()

  ret = {'scores': scores, 'clses': clses.float(), 
         'xs': xs0, 'ys': ys0, 'cts': cts, 'visibility': visibility}

  if 'visibility' in output:
    visibility = _tranpose_and_gather_feat(output['visibility'], inds)
    visibility = visibility.view(batch, K, -1)
    ret['visibility'] = visibility

  if 'reg' in output:
    reg = _tranpose_and_gather_feat(output['reg'], inds)
    reg = reg.view(batch, K, 2)

    xs = xs0.view(batch, reg.size(1), 1) + reg[:, :, 0:1]
    ys = ys0.view(batch, reg.size(1), 1) + reg[:, :, 1:2]
  else:
