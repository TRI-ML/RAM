    ...
    def imgpre2feats(self, x, pre_img=None, pre_hm=None):
        if self.opt.is_recurrent:            
            if type(x) == list:
                ...
                inp_len = 1
                batch_size = len(x[0]['image'])
                if not self.opt.stream_test:
                    # inp_len = self.opt.input_len
                    inp_len = len(x)

                if self.opt.split_batch and inp_len != 1:
                    update_part = random.randint(0, 1)
                    slice_len = int(inp_len / 2)

                    if update_part == 0:
                        x = inp[:, :slice_len, :, :, :]
                    else:
                        x = inp[:, slice_len:, :, :, :]
                    x = x.reshape(batch_size * slice_len, inp.size(2), inp.size(3), inp.size(4)) 
                    with torch.no_grad():
                        y1 = self.do_tensor_pass(x, pre_img, pre_hm)
                    ...
                    x = x.reshape(batch_size * slice_len, inp.size(2), inp.size(3), inp.size(4))  
                    y2 = self.do_tensor_pass(x, pre_img, pre_hm)

                    y1 = y1.view(batch_size, slice_len, y1.size(-3), y1.size(-2), y1.size(-1))
                    y2 = y2.view(batch_size, slice_len, y1.size(-3), y1.size(-2), y1.size(-1))

                    if update_part == 0:
                        y = torch.cat((y1, y2), 1)
                    else:
                        y = torch.cat((y2, y1), 1)
                    y = y.view(batch_size * inp_len, y.size(-3), y.size(-2), y.size(-1))
                else:
