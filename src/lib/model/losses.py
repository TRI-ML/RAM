...
class FastFocalLoss(nn.Module):
  ...
  def forward(self, out, target, ind, mask, cat, zero_neg=False):
    '''
    Arguments:
    ...
    per_vid_loss = pos_loss.sum(1)
    pos_loss = pos_loss.sum()
    
    if num_pos == 0:
      return - neg_loss, per_vid_loss * 0
    return - (pos_loss + neg_loss) / num_pos, per_vid_loss / num_pos
...
class WeightedBCELossNoLogits(nn.Module):
  def __init__(self):
    super(WeightedBCELossNoLogits, self).__init__()
