...
class Tao:
    ...
    def group_ann_tracks(self, anns, use_gt_ignore=True):
        max_track_id = -1
        for ann in anns:
            if ann['track_id'] > max_track_id:
                max_track_id = ann['track_id']

        count_ignore = 0
        count_invis_tracks = 0
        count_small_tracks = 0
        if use_gt_ignore:
            for ann in anns:
                if 'ignore' in ann and ann['ignore'] == 1:
                    ann['track_id'] += max_track_id + 1
                    count_ignore += 1

                    if ann['track_id'] not in self.tracks:
                        # if ann['occlusion'] < 0.05:
                        #     count_invis_tracks += 1
                        # else:
                        #     count_small_tracks += 1
                        old_track = self.tracks[ann['track_id'] - max_track_id - 1]
                        self.tracks[ann['track_id']] = {'id': ann['track_id'], 'category_id': old_track['category_id'], 'video_id': old_track['video_id']}
        print("%d boxes ignored" % count_ignore)
        print("%d invis tracks hidden" % count_invis_tracks)
        print("%d small tracks hidden" % count_small_tracks)
        ...
        for tid, track in tracks.items():
            ...
            if use_gt_ignore and 'ignore' in track['annotations'][0] and track['annotations'][0]['ignore'] == 1:
                track['ignore'] = 1
                count_ignore += 1
        print("%d tracks ignored" % count_ignore)
